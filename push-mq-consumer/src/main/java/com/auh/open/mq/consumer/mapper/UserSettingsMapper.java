...
public interface UserSettingsMapper extends BaseMapper<UserSettings> {
    ...
    @Select("select user_push_settings from chouti_user_settings where jid=#{jid} limit 1")
    Long getUserPushSettings(@Param("jid") String jid);

    /**
     * 与运算,查询设置匹配的用户
     */
    @Select("select jid from chouti_user_settings where user_push_settings & #{user_push_settings}")
